...
class DecodeBox():
    ...
    def get_top_three_labels(self, label_array):
        color_array = [0, 3, 4, 6, 7, 15, 16, 17, 21, 22, 23, 24]
        orientation_array = [1, 5, 8, 13]
        type_array = [2, 9, 10, 11, 12, 14, 18, 19, 20]
        color_top_score, color_top = torch.max(label_array[:, color_array], 1, keepdim=True)
        orientation_top_score, orientation_top = torch.max(label_array[:, orientation_array], 1, keepdim=True)
        type_top_score, type_top = torch.max(label_array[:, type_array], 1, keepdim=True)

        class_score_collections = [[
            type_array[type_top[i][0].item()],
            type_top_score[i][0].item(),
            color_array[color_top[i][0].item()],
            color_top_score[i][0].item(),
            orientation_array[orientation_top[i][0].item()],
            orientation_top_score[i][0].item(),
            torch.mean(torch.as_tensor([type_top_score[i][0].item(), color_top_score[i][0].item(), orientation_top_score[i][0].item()]))
        ] for i in range(len(color_top))]

        category_encoding = np.asarray([str(type_array[type_top[i][0].item()]) + "," + str(color_array[color_top[i][0].item()]) + "," +
            str(orientation_array[orientation_top[i][0].item()]) for i in range(len(color_top))])

        return torch.from_numpy(np.array(class_score_collections)).cuda(), category_encoding

    def non_max_suppression_multi_label(self, prediction, num_classes, input_shape, image_shape, letterbox_image,
        ...
        for i, image_pred in enumerate(prediction):
            multi_class_score_collection, category_encodings = self.get_top_three_labels(
                image_pred[:, 5:5 + num_classes])

            # 第一轮筛选 框置信度*平均类别置信度
            conf_mask1 = (image_pred[:, 4] * multi_class_score_collection[:, -1] >= conf_thres).squeeze()

            image_pred1 = image_pred
            image_pred1 = image_pred1[conf_mask1]
            multi_class_score_collection = multi_class_score_collection[conf_mask1]
            category_encodings = category_encodings[conf_mask1.cpu()]
            if not image_pred.size(0):
                continue

            detections1 = torch.cat((image_pred1[:, :5], multi_class_score_collection.float()), 1)

            unique_labels1 = list(set(category_encodings))

            for c in unique_labels1:
                label_indexes = np.expand_dims(np.where(category_encodings == c), axis=1)
                detection_class1 = detections1[label_indexes].squeeze(0)

                keep = nms(
                    detection_class1[:, :4],
                    detection_class1[:, 4] * detection_class1[:, -1],
                    nms_thres
                )

                max_detections = detection_class1[keep]
