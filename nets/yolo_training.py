...
class YOLOLoss(nn.Module):
    ...
    def binary_truncation(self, values):
        new_values = torch.where(values >= 0.0, 1.0, 0.0)
        return new_values

    def Punish(self, pred, target, T):
        output = 1.0 - self.binary_truncation(1.0-T-target) * self.binary_truncation(1.0-T-pred) - \
                 self.binary_truncation(target-T) * self.binary_truncation(pred-T)
        return output

    def Punish_BCELoss(self, pred, target, T=0.5):
        epsilon = 1e-7
        pred = self.clip_by_tensor(pred, epsilon, 1.0 - epsilon)
        output = - self.Punish(pred, target, T) * target * torch.log(pred) - self.Punish(pred, target, T) \
    ...
    # yolo_loss(l, outputs[l], targets, y_trues[l], epoch)
    def forward(self, l, input, targets=None, y_true=None, epoch=None):
        #----------------------------------------------------#
        #   l               代表使用的是第几个有效特征层
        ...
        if n != 0:
            ...
            if epoch <= 250:
                ...
                loss_cls = torch.mean(self.Punish_BCELoss(pred_cls[y_true[..., 4] == 1],
